...
export default function EscapeRoom() {
  ...
  function portCsvToJsonAction() {
    ...
    const json = rest.map((r) => {
      ...
      return Object.fromEntries(
        cols.map((c, idx) => [c, isNaN(Number(vals[idx])) ? vals[idx] : Number(vals[idx])])
      );
    });
  ...
  function onRoomClick(e: React.MouseEvent) {
    ...
    const correct =
      ...
      x < box.width * 0.68 + 60 &&
      y > box.height * 0.42 &&
      y < box.height * 0.42 + 60;
    debugClickAction(correct);
  }

  // --- expose auto helper for Playwright ---
  useEffect(() => {
      ...
      <div className="escape-row stages">
        <ol className="stage-list">
          {STAGES.map((s, idx) => (
            <li key={s.key}>
              <button
                className={idx === i ? 'stage active' : 'stage'}
                onClick={() => setI(idx)}
              >
                {s.title}
              </button>
        ...
        <div className="stage-content">
          ...
          {STAGES[i].key === 'format' && (
            <div>
              ...
              <button onClick={formatCodeAction} data-testid="run-format">
                Run
              </button>
            </div>
          )}
          ...
          {STAGES[i].key === 'numbers' && (
            <div>
              ...
              <button onClick={numbersAction} data-testid="run-numbers">
                Run
              </button>
            </div>
          )}
          ...
          {STAGES[i].key === 'port' && (
            <div>
              ...
              <button onClick={portCsvToJsonAction} data-testid="run-port">
                Run
              </button>
            </div>
          )}
          ...
          <div className="output-wrap">
            ...
            <pre className="output" data-testid="output" aria-live="polite">
              {output}
            </pre>
          </div>

          <div className="actions">
            <button onClick={saveSnippet} data-testid="save">
              Save Output
            </button>
            {msg && (
              <span className="msg" data-testid="status">
                {msg}
              </span>
            )}
          </div>
